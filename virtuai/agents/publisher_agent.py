...
Two parallel tool tiers are exposed to the agent:
  ...
  TIER 1 — Composio dynamic tools (flexible, multi-platform):
    ...
    FACEBOOK_CREATE_POST + FACEBOOK_CREATE_PHOTO_POST + FACEBOOK_CREATE_VIDEO_POST
    (X / Twitter dropped 2026-05-21; Facebook is now the 4th platform.)
...
in this run includes:
  ...
  - FACEBOOK_CREATE_PHOTO_POST       →  Facebook Page photo (Composio)
  - FACEBOOK_CREATE_VIDEO_POST       →  Facebook Page video (Composio)
...
PUBLISH SAFETY GATES — DO NOT PUBLISH unless ALL of these are true:
  ✓ Research recommendation is 'continue' (not 'skip')
  ✓ Reviewer approval_status is 'approve'
  ✓ Guardian safety_status is 'safe'
  ✓ final_decision is 'approved'
  ✓ publish_ready is true
  ✓ Required rendered files exist on disk and are > 0 bytes
  ✓ Caption / metadata are present and non-empty
  ✓ Manual approval is NOT blocking automatic publishing
If ANY gate fails: do NOT call any Composio or YouTube tool. Return
publisher_status="skipped" with the failed gate(s) in the errors list.

Manual approval: when context["manual_approval"] is true, stage the
package but do not actually call Composio or YouTube. Return
publisher_status="manual_approval_required" with the staged files in
files_published and the caption in caption_published.

Dry-run: when context["dry_run"] is true, return
publisher_status="dry_run" without calling any external tool. Files
are inspected for existence but never posted.

STRUCTURED OUTPUT — emit ONE JSON object per published (or attempted)
item, with this exact shape:
{
  "publisher_status":     "published"|"scheduled"|"failed"|"manual_approval_required"|"skipped"|"dry_run",
  "platform":             "instagram"|"linkedin"|"facebook"|"youtube_shorts",
  "content_type":         "reel"|"portrait"|"carousel"|"post"|"short",
  "published_url":        "<final URL or empty>",
  "scheduled_time":       "<ISO time or empty>",
  "composio_action_used": "<tool name or empty>",
  "youtube_upload_used":  true | false,
  "files_published":      ["/abs/path/..."],
  "caption_published":    "<the literal caption>",
  "errors":               ["<gate or API failure>"],
  "logs_saved_to":        ["virtuai/data/logs/..."],
  "next_step":            "<one-line follow-up>"
}
...
def make_publisher(llm: LLM) -> Agent:
    ...
    The agent's tool list is Composio's tools (LinkedIn / Facebook / Instagram) PLUS our direct YouTube tool — Composio's YOUTUBE_UPLOAD_VIDEO
