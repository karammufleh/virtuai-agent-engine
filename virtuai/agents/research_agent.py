...
def create_research_agent(llm: LLM) -> Agent:
    return Agent(
        ...
        backstory=(
            ...
            "MANDATORY ANCHORS in your final output:\n"
            ...
            "All scores are 0-10 (you assign them based on the data you saw):\n"
            "{\n"
            '  "trend_found":               true | false,\n'
            '  "trend_name":                "<topic>",\n'
            '  "trend_summary":             "<2-3 sentences>",\n'
            '  "niche":                     "AI + automation in business",\n'
            '  "why_it_is_trending":        "<1-2 sentences>",\n'
            '  "sources":                   ["<launch / thread / news ref>"],\n'
            '  "keywords":                  ["..."],\n'
            '  "hashtags":                  ["#..."],\n'
            '  "platforms_where_seen":      ["instagram"|"linkedin"|"facebook"|"youtube_shorts"],\n'
            '  "freshness_score":           0..10,\n'
            '  "niche_relevance_score":     0..10,\n'
            '  "virality_score":            0..10,\n'
            '  "platform_relevance_score":  0..10,\n'
            '  "persona_fit_score":         0..10,\n'
            '  "risk_score":                0..10  // higher = more risky,\n'
            '  "content_opportunity_score": 0..10,\n'
            '  "recommendation":            "continue" | "skip",\n'
            '  "reason":                    "<one-line why>"\n'
            "}\n\n"
            "SKIP RULES (emit recommendation='skip' if ANY are true):\n"
            "  - No fresh signal exists (freshness < 4)\n"
            "  - Topic is off-niche (niche_relevance < 4)\n"
            "  - Topic carries serious risk (risk_score > 7)\n"
            "  - All 10 candidates collapse into a banned pattern\n"
            "  - score_topic_virality total < 22/40 even after fixes\n\n"
            "When skipping, the pipeline stops cleanly — never force a weak\n"
            "topic just to keep the cron schedule fed."
        ),
        llm=llm,
        tools=[
            fetch_industry_signals,
