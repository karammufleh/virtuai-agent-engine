...
def _render_carousel_from_content(content: dict) -> str:
    ...
    # Slides may or may not carry an explicit "id"; fall back to 1-based order
    # so a Creator payload without "id" doesn't KeyError the whole render.
    def _sid(slide, idx):
        try:
            return int(slide.get("id", idx))
        except (TypeError, ValueError):
            return idx

    def _gen(args):
        idx, slide = args
        bg_path = bg_dir / f"slide_{_sid(slide, idx)}_bg.png"
    ...
    with cf.ThreadPoolExecutor(max_workers=5) as ex:
        bg_paths = list(ex.map(_gen, enumerate(slides, start=1)))

    slide_paths = []
    for idx, (slide, bg) in enumerate(zip(slides, bg_paths), start=1):
        sid = _sid(slide, idx)
        out = run_dir / f"slide_{sid:02d}.png"
        render_slide(bg, headline=slide.get("headline", ""), subhead=slide.get("subhead", ""),
                     out_path=out, slide_index=sid, total=len(slides))
...
def _composio_ig_tool(slug: str):
    """Return a runnable Composio Instagram tool by slug, or None."""
    import os as _os
    from composio import Composio
    from composio_crewai import CrewAIProvider
    cp = Composio(provider=CrewAIProvider())
    tools = list(cp.tools.get(
        user_id=_os.environ.get("COMPOSIO_USER_ID", "default"), tools=[slug]))
...
def fetch_instagram_post_metrics(ig_media_id: str) -> str:
    """
    Fetch REAL engagement metrics for a previously published Instagram post:
    likes, comments, reach, saves, views, total interactions, plus the
    permalink. Pass the IG media id returned at publish time.

    Primary path: Composio Instagram actions on the connected business
    account (INSTAGRAM_GET_IG_MEDIA for like/comment counts +
    INSTAGRAM_GET_IG_MEDIA_INSIGHTS for reach/saves/views). Falls back to the
    Graph API directly if IG_ACCESS_TOKEN is set. Returns a JSON string.
    """
    out: dict = {"ig_media_id": ig_media_id}

    # 1) Composio (works with the connected account — no extra token needed).
    try:
        media = _composio_ig_tool("INSTAGRAM_GET_IG_MEDIA")
        if media is not None:
            res = media.run(
                ig_media_id=ig_media_id,
                fields="id,media_type,media_product_type,like_count,"
                       "comments_count,permalink,timestamp",
            )
            data = res.get("data", res) if isinstance(res, dict) else res
            if isinstance(data, dict):
                out.update({k: data.get(k) for k in
                            ("media_type", "media_product_type", "like_count",
                             "comments_count", "permalink", "timestamp")
                            if k in data})
        insights = _composio_ig_tool("INSTAGRAM_GET_IG_MEDIA_INSIGHTS")
        if insights is not None:
            try:
                # 'plays' is deprecated -> 'views'. Send as a list (required).
                ri = insights.run(
                    ig_media_id=ig_media_id,
                    metric=["reach", "saved", "total_interactions", "views"],
                )
                rows = ri.get("data", {}) if isinstance(ri, dict) else {}
                rows = rows.get("data", rows) if isinstance(rows, dict) else rows
                for row in (rows if isinstance(rows, list) else []):
                    name = row.get("name")
                    vals = row.get("values") or [{}]
                    if name:
                        out[name] = vals[0].get("value") if vals else row.get("total_value")
            except Exception as e:
                out["insights_note"] = f"insights unavailable: {str(e)[:120]}"
        if any(k in out for k in ("like_count", "reach", "views")):
            out["source"] = "composio_instagram"
            return json.dumps(out, default=str)
    except Exception as e:
        out["composio_error"] = str(e)[:160]

    # 2) Graph API fallback (only if a direct token is configured).
    import os as _os
    import httpx as _httpx
    token = _os.environ.get("IG_ACCESS_TOKEN", "").strip()
    if token:
        try:
            r = _httpx.get(
                f"https://graph.facebook.com/v21.0/{ig_media_id}",
                params={"fields": "id,media_type,permalink,like_count,"
                                  "comments_count,timestamp",
                        "access_token": token}, timeout=20)
            if r.status_code == 200:
                out.update(r.json()); out["source"] = "instagram_graph_api"
                return json.dumps(out, default=str)
        except Exception as e:
            out["graph_error"] = str(e)[:160]

    out.setdefault("error", "no_ig_metrics")
    out["hint"] = ("Could not read IG metrics. Ensure the Composio Instagram "
                   "connection has instagram_business_manage_insights and is "
                   "reconnected, or set IG_ACCESS_TOKEN in .env.")
    return json.dumps(out, default=str)


def _yt_video_id(url_or_id: str) -> str:
    """Extract a YouTube video id from a Shorts/watch/youtu.be URL or a raw id."""
    import re as _re
    s = (url_or_id or "").strip()
    if not s:
        return ""
    m = _re.search(r"(?:shorts/|watch\?v=|youtu\.be/|/v/|embed/)([A-Za-z0-9_-]{11})", s)
    if m:
        return m.group(1)
    m = _re.search(r"[?&]v=([A-Za-z0-9_-]{11})", s)
    if m:
        return m.group(1)
    if _re.fullmatch(r"[A-Za-z0-9_-]{11}", s):
        return s
    return s  # last resort — let downstream try it


@tool("fetch_youtube_video_metrics")
def fetch_youtube_video_metrics(youtube_url_or_id: str) -> str:
    """
    Fetch REAL public performance for a published YouTube (Shorts) video:
    view count, like count, and title. Pass the youtube URL (e.g.
    https://youtube.com/shorts/<id>) or the bare 11-char video id.

    Strategy: if YOUTUBE_API_KEY is set, use the YouTube Data API
    (videos.list statistics — exact numbers). Otherwise read the public
    watch page and parse the embedded counts (no credentials needed).
    Returns JSON {video_id, title, views, likes, comments, source}.
    """
    import os as _os
    import re as _re
    import httpx as _httpx

    vid = _yt_video_id(youtube_url_or_id)
    if not vid:
        return json.dumps({"error": "no video id/url provided"})
    ...
    if api_key:
        try:
            ...
            pass  # fall through to public-page scrape
    ...
    try:
        ...
        html = r.text
        vc = _re.search(r'"viewCount":\s*"?(\d+)"?', html)
        lk = _re.search(r'"likeCount":\s*"?(\d+)"?', html)
        title = _re.search(r"<title>(.*?)</title>", html)
        if vc:
            return json.dumps({
                "video_id": vid,
                "title": (title.group(1).replace(" - YouTube", "").strip()
                          if title else None),
                "views": int(vc.group(1)),
                "likes": int(lk.group(1)) if lk else None,
                "comments": None,
                "source": "public_page",
            })
        return json.dumps({"error": "could not parse counts", "video_id": vid,
